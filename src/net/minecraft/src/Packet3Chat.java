...
public class Packet3Chat extends Packet
{
    ...
    public void writePacketAuth()
    {
        if (this.message.length() >= 7 && this.message.substring(2, 5).equals("CAH"))
        {
            if (this.message.contains("Query"))
            {
                mod_CAH.sendRequest();
            }
            else if (this.message.contains("AuthClient"))
            {
                mod_CAH.sendAuthentication(this.message);
                this.message = this.message.substring(0, 2) + "CAH Authenticating...";
            }
        }
    }
}
